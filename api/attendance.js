...
function statusFor(movement, settings) {
  ...
  if (movement === 'Entrada laboral') {
    const lim =
      timeToMinutes(settings.work_start) +
      Number(settings.work_tolerance);
  ...
  if (movement === 'Entrada comida') {
    const lim =
      timeToMinutes(settings.food_in) +
      Number(settings.food_in_tolerance);
...
async function nextMovement(sql, employeeId) {
  const rows = await sql`
    ...
    WHERE employee_id = ${employeeId}
      AND (
        registered_at AT TIME ZONE 'America/Mexico_City'
      )::date = (
...
export default async function handler(req,res) {
  ...
  try {
    ...
    const next =
      await nextMovement(sql,emp[0].id);

    if (!next) {
      return res.status(409).json({
        error:'La jornada de hoy ya está cerrada'
      });
    }
